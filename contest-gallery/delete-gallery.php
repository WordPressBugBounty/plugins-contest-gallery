...
if (!empty($_GET['option_id']) AND !empty($_POST['cg_delete_gallery'])) {
	...
	foreach($EcommerceDownloadEntries as $EcommerceEntry){
		cg_move_file_from_ecommerce_sale_folder($EcommerceEntry->pid, $EcommerceEntry->GalleryID,$EcommerceEntry->id,false,true);
	}
    ...
    if(intval($galleryVersion)>=21 || !empty($options->WpPageParent)){
        ...
        foreach ($rowObjects as $rowObject){
            ...
            if(!empty($rowObject->WpPageEcommerce)){wp_delete_post($rowObject->WpPageEcommerce,true);}
        }
    }
    ...
    $wpdb->query($wpdb->prepare(
        "
				DELETE FROM $tablename_ecommerce_entries WHERE GalleryID = %d
			",
        $optionID
    ));
    ...
    if(!empty($options->WpPageParentEcommerce)){
        $wpdb->query($wpdb->prepare(
            "
				DELETE FROM $tablename_wp_pages WHERE WpPage = %d
			",
            $options->WpPageParentEcommerce
        ));
        wp_delete_post($options->WpPageParentEcommerce,true);
    }
