<?php
if(!function_exists('cg_stripe_create_customer')){
    function cg_stripe_create_customer($email,$secret){
	    ...
	    if (curl_errno($ch)) {
		    echo 'Error cg_stripe_create_customer:' . curl_error($ch);
	    }else{
		    if(!empty($result['error']['message'])){
			    echo 'Error cg_stripe_create_customer:' . $result['error']['message'];
		    }
	    }
