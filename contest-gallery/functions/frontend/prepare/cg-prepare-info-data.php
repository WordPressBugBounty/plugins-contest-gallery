...
if (!function_exists('cg1l_build_images_info_data_gzip')) {
    function cg1l_build_images_info_data_gzip($gid, $imagesFullData = [], $getDataOnly = false) {

        $wp_upload_dir = wp_upload_dir();

        $base_dir = $wp_upload_dir['basedir'].'/contest-gallery/gallery-id-'.$gid.'/json/segments';
        $final    = $base_dir . "/images-info-data.json.gz.php";
        $lastUpdated    = $base_dir . "/image-info-data-last-update.txt";
        ...
        if($getDataOnly && file_exists($final) && !$newBuilt) {
            ...
            if($fh) {
                ...
                $jsonInfoData = json_decode($json, true);
                if(empty($jsonInfoData)) {
                    $jsonInfoData = [];
                }
                return $jsonInfoData;
            }
        ...
        if($newBuilt){
            $infoDataJsonFiles = glob($wp_upload_dir['basedir'].'/contest-gallery/gallery-id-'.$gid.'/json/image-info/*.json');
            $jsonInfoData = [];
            ...
            //$temp = $base_dir . '/images-info-data.tmp';
            ...
            return cg1l_build_images_info_data_gzip($gid, $imagesFullData, true);
        }
