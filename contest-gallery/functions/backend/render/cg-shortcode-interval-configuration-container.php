...
if(!function_exists('cg_shortcode_interval_configuration_container')){
    function cg_shortcode_interval_configuration_container($GalleryID,$cgProFalse){
	    ...
	    if(isset($optionsJson['interval']['cg_google_sign_in'])){unset($optionsJson['interval']['cg_google_sign_in']);}
	    ...
	    $optionsIntervalGeneral = json_decode(file_get_contents($optionsPath),true);
	    $intervalOptionsMerged = array_merge($optionsIntervalGeneral['interval'], $optionsJson['interval']);
	    $optionsJson['interval'] = $intervalOptionsMerged;
