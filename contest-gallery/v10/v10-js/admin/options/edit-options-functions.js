...
cgJsClassAdmin.options.functions = {
    ...
    initOptionsClickEvents: function (isShortcodeConf,isEcommerceConf,$cgSellContainer) {

        function cgViewOptionCheck(e){
        ...
        }else if(isEcommerceConf){
            $cgSellContainer.find('.cg_view_option').each(function (){
              //  jQuery(this).get(0).removeEventListener('click', cgViewOptionCheck);
            });
            var $classElements = $cgSellContainer.find('.cg_view_option');
        }else{
...
/*            classElements[i].addEventListener("click", function (e){
    ...
    loadOptionsArea: function ($,$formLinkObject,$response) {
        ...
        /*var $cgEditOptionsForm = $('#cgEditOptionsForm');

        if($formLinkObject.hasClass('cg_edit_translations') || location.hash.indexOf('edit_translations')>-1){

            if(!location.hash.indexOf('edit_translations=true')>-1){
                cgJsClassAdmin.gallery.vars.isHashJustChanged = true;
                location.hash = location.hash+'&edit_translations=true';
            }
            $cgEditOptionsForm.addClass('cg_edit_translations');
            $cgEditOptionsForm.find('.cg_view_select:not(.cg_view_select_translations)').addClass('cg_hide_override');
            $cgEditOptionsForm.find('.cg_view:not(.cg_view_translations)').addClass('cg_hide');
            $cgEditOptionsForm.find('.cg_view_header:not(.cg_view_header_translations)').addClass('cg_hide');
            $cgEditOptionsForm.find('.cg_gallery_view_note').addClass('cg_hide_override');
            $cgEditOptionsForm.find('.cg_entry_view_note').addClass('cg_hide_override');
            $cgEditOptionsForm.find('.cg_short_code_multiple_pics_configuration_buttons').addClass('cg_hide');
            $cgEditOptionsForm.find('.cg_short_code_single_pic_configuration_buttons').addClass('cg_hide');
            $cgEditOptionsForm.find('#cgSaveOptionsNavButton strong').text('Save all translations');
            $cgEditOptionsForm.find('#cgSaveOptionsButton').val('Save all translations');
            $cgEditOptionsForm.find('.cg_view_header.cg_view_header_translations').css({
                'margin-top':'0',
                'border-top':'unset'
            });
            $cgEditOptionsForm.find('#cg_main_options_tab_first_row .cg_view_select.cg_view_select_translations').insertBefore($cgEditOptionsForm.find('#cg_main_options_tab_second_row .cg_view_select.cg_view_select_translations'));
            $cgEditOptionsForm.find('#cg_main_options_tab_first_row').addClass('cg_hide');
            $cgEditOptionsForm.prepend('<input type="hidden" name="cg_edit_translations" value="true" />');
        }else if($formLinkObject.hasClass('cg_edit_ecommerce') || location.hash.indexOf('cg_edit_ecommerce')>-1){
            //
        }else{// then simply must be edit options
            $cgEditOptionsForm.addClass('cg_edit_options');
            $cgEditOptionsForm.find('.cg_view_select_translations').addClass('cg_hide_override');
            $cgEditOptionsForm.find('.cg_view_translations').addClass('cg_hide');
            $cgEditOptionsForm.find('.cg_view_header_translations').addClass('cg_hide');
        }*/
        ...
        var goToArray = ['cgTranslationOther','cgTranslationShippingPossible','cgTranslationLanguageEmail','cgTranslationLanguageEmail','cgEditGalleryNameRow','cgTranslationLanguagePassword','l_ThankVote','l_BackToGallery','l_BackToGalleries','TranslationsCommentFormArea','l_AllVotesUsed','YourCommentWillBeReviewedRow','cgContactFormShortcodeConfigurationArea','l_GalleryDateFormat','cgOpenAiKeyRowColumn'];
        debugger
        $(goToArray).each(function (index,value){
            if(location.hash.indexOf(value) >= 0 || location.search.indexOf(value) >= 0){
                ...
                var $cgGoTo = $('#'+value);
                if($cgGoTo.length){
                    cgJsClassAdmin.index.functions.cgGoTo($cgGoTo);
                }else{
                    $cgGoTo = $('[data-cg-go-to-target="'+value+'"]');
                    if($cgGoTo.length){
                        cgJsClassAdmin.index.functions.cgGoTo($cgGoTo);
                    }
                }
...
//$( "#ScaleSizesGalery" ).attr("disabled",true);
        ...
        if($Tax.length){
            var PriceDivider = $('#PriceDivider').val();
            if(PriceDivider=='.'){
            ...
            cgJsClassAdmin.options.functions.setCurrencyTaxFieldOnLoad();
    ...
    cgViewOptionCheck: function (element,e){
        ...
        if($element.hasClass('cg_key_upload_option')){
            return;
        }

        if($element.hasClass('cg_view_option_not_focus')){
            return;
        }
        ...
        if($(e.target).hasClass('cg_event_link')){
            return;
        }
   ...
   /* cg_RatingVisibleForGalleryNoVotingLoop: function ($) {
    ...
    changeCurrencyField: function ($element,priceDivider,thousandsDivider){
        ...
        if(input_val!=''){
            var decimal_pos = input_val.indexOf(priceDivider);
        }

        if(input_val!=''){
            input_val = cgJsClassAdmin.gallery.currencyInputFunctions.correctInputVal(decimal_pos,input_val,undefined,priceDivider,thousandsDivider);
            $element.val(input_val);
            cgJsClassAdmin.gallery.currencyInputFunctions.setCurNew($element,undefined,undefined,undefined,priceDivider);
        }
    },
    setCurrencyTaxFieldOnLoad: function (){
        var $Tax = jQuery('#TaxPercentageDefault');
        if($Tax.val()==''){$Tax.val(0);}
        var input_val = String($Tax.val());
        if(cgJsClassAdmin.gallery.currencyInputFunctions.priceDivider==','){
            input_val = input_val.replace('.',',');
        }
        $Tax.val(input_val);
        cgJsClassAdmin.gallery.currencyInputFunctions.setCurNew($Tax,undefined,undefined,undefined);

    },
    setCurrencyShippingFieldOnLoad: function (){
        var $Shipping = jQuery('#Shipping');
        if($Shipping.val()==''){$Shipping.val(0);}
        var input_val = String($Shipping.val());
        if(cgJsClassAdmin.gallery.currencyInputFunctions.priceDivider==','){
            input_val = input_val.replace('.',',');
        }
