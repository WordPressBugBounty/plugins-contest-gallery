...
cgJsClassAdmin.index.functions = {
    ...
    cgLoadBackend: function ($formLinkObject,isSaveData,isShowNoLoader,isDoNotEmptyContent,isCopyGallery,isCopyGalleryNextProcess) {
        ...
        if($formLinkObject.is('form')){
            ...
            if($formLinkObject.attr('id')=='cgEditOptionsForm' && $formLinkObject.find('#AllowedCountriesContainer').length){
                isEditOptionsSubmit = true;
            }
     ...
     //   $formLinkObject.addClass('cg_hide');
        ...
        if($formLinkObject.is('form')){
            ...
            // convert currency first for saving
            var form = $formLinkObject.get(0);
        ...
        var cgBackendHashVal = cgJsClassAdmin.index.functions.cgLoadBackendLoader(isShowNoLoader,isDoNotEmptyContent);
           ...
           // var cgBackendHashVal = $cg_main_container.find('#cgBackendHash').val();
            ...
            if(!isCopyGalleryNextProcess){
                jQuery('<h2 class="cg_first_in_progress">In progress ...</h2><p class="cg_first_in_progress">Do not cancel</p>').insertBefore(jQuery('#cgBackendBackgroundDrop'));
            }
           jQuery('body').removeClass('cg_no_scroll');
           jQuery('#cg_main_container').get(0).scrollIntoView();
        }

        formPostData.append('cgBackendHash',cgBackendHashVal);

        if(isEditOptionsSubmit && cgJsClassAdmin.options.vars.AllowedCountriesTranslations ){
            cgJsClassAdmin.options.vars.AllowedCountriesTranslations.length = 1;// otherwise will be not iterated
            debugger
            for(var index in cgJsClassAdmin.options.vars.AllowedCountriesTranslations){
                if(!cgJsClassAdmin.options.vars.AllowedCountriesTranslations.hasOwnProperty(index)){
                    break;
                }
                debugger
                if(cgJsClassAdmin.options.vars.AllowedCountriesTranslations[index]){
                    debugger
                    for(var index1 in cgJsClassAdmin.options.vars.AllowedCountriesTranslations[index]){
                        if(!cgJsClassAdmin.options.vars.AllowedCountriesTranslations[index].hasOwnProperty(index1)){
                            break;
                        }
                        debugger
                        var value1 = cgJsClassAdmin.options.vars.AllowedCountriesTranslations[index][index1];
                        formPostData.append('AllowedCountriesTranslations['+index+']['+index1+']',value1);
                    }
                }
           }
    ...
    getWpVersionAsInteger: function () {
        ...
        var wpVersionInt = 0;
        if(wpVersion){
            wpVersionInt = parseInt(wpVersion.replace('.','').replace('.','').replace('.','').replace('.','').replace('.','').replace('-RC1',''));
