...
if(!empty($_POST['upload'])){
	...
	if(!empty($_POST['deleteFieldnumber'])){
		...
		if(is_array($_POST['deleteFieldnumber'])){
			...
			$deletedFieldnumberInput = $deleteFieldnumber;
		}
		...
		foreach($_POST['upload'] as $id => $field){
			...
			if($field['type']=='nf' OR $field['type']=='fbt'){// TEXT FIELD!!!!!
				...
				if(empty($field['new'])){
					...
					if(!empty($EcommerceTitle)){$EcommerceTitleToSet=$id;}
					if(!empty($EcommerceDescription)){$EcommerceDescriptionToSet=$id;}
					...
					if(!empty($EcommerceTitle)){$EcommerceTitleToSet=$wpdb->insert_id;}
					if(!empty($EcommerceDescription)){$EcommerceDescriptionToSet=$wpdb->insert_id;}
