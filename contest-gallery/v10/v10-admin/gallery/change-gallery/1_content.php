...
if (!empty($content)) {
	...
	foreach($content as $key => $arrayValue){
		...
		if(!empty($realIdWhereSourceChanged) && $realIdWhereSourceChanged==$imageId){
			//var_dump('$content continue');
			// because if replace or add empty content fields will be saved
			//	continue;
		}
		...
		foreach($arrayValue as $arrayKey => $value){
			...
			// is for version before 22 wp_update_post and post_title and change guid all that will be done in
			// cg_json_upload_form_info_data_files_new
			if(intval($galeryrow->Version)<22 && !empty($IsForWpPageTitleInputId) && $formFieldId==$IsForWpPageTitleInputId){
				$entryRow = $wpdb->get_row($wpdb->prepare("SELECT WpPage, WpPageUser, WpPageNoVoting, WpPageWinner, WpPageEcommerce FROM $tablename WHERE id = %d",[$imageId]));
				...
				if(!empty($entryRow->WpPage)){
					$post_update = array(
						'ID'         => $entryRow->WpPage,
						'post_title' => $field_content_original
					);
				...
				if(!empty($entryRow->WpPageEcommerce)){
					$post_update = array(
						'ID'         => $entryRow->WpPageEcommerce,
						'post_title' => $field_content_original
					);
					wp_update_post( $post_update );
				}
	...
	if(empty($isFromFrontendGalleryImageEdit) && !empty($infoPidsArray)){
		if(!empty($hasRealIdDeleted)){
			// has to actualize all then
			cg_json_upload_form_info_data_files_new($GalleryID,$infoPidsArray,true,false,true);
			// has to be done extra here, check with not empty required
			cg_json_upload_form_info_data_files_new($GalleryID,[$realIdWhereSourceChanged],true,false,true);
		}else{
			cg_json_upload_form_info_data_files_new($GalleryID,$infoPidsArray,true);
		}
	}
