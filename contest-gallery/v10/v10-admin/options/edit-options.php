...
$editTranslationLink = '?page='.cg_get_version().'/index.php&edit_options=true&option_id='.$GalleryID.'&cg_edit_translations=true&cg_go_to=';
...
if(floatval($galleryDbVersion)>=22){
    ...
    $CheckoutAgreementOne = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->CheckoutAgreementOne);
    $CheckoutAgreementTwo = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->CheckoutAgreementTwo);
    $CheckoutAgreementThree = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->CheckoutAgreementThree);
    $CheckoutNoteTop = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->CheckoutNoteTop);
    $CheckoutNoteBottom = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->CheckoutNoteBottom);
    $OrderConfirmationMail = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->OrderConfirmationMail);
	$RegUserPurchaseOnly = ($selectSQLecommerceOptions->RegUserPurchaseOnly==1) ? 'checked' : '';
	$RegUserPurchaseOnlyText = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->RegUserPurchaseOnlyText);
	$RegUserPurchaseOnlyTextDisabled = ($selectSQLecommerceOptions->RegUserPurchaseOnly==1) ? '' : 'cg_disabled';
	$RegUserOrderSummaryOnly = ($selectSQLecommerceOptions->RegUserOrderSummaryOnly==1) ? 'checked' : '';
	$RegUserOrderSummaryOnlyText = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->RegUserOrderSummaryOnlyText);
	$RegUserOrderSummaryOnlyTextDisabled = ($selectSQLecommerceOptions->RegUserOrderSummaryOnly==1) ? '' : 'cg_disabled';

	$SendOrderConfirmationMail = ($selectSQLecommerceOptions->SendOrderConfirmationMail==1) ? 'checked' : '';
	$OrderConfirmationMailHeader = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->OrderConfirmationMailHeader);
    $OrderConfirmationMailReply = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->OrderConfirmationMailReply);
    $OrderConfirmationMailBcc = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->OrderConfirmationMailBcc);
    $OrderConfirmationMailCc = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->OrderConfirmationMailCc);
    $OrderConfirmationMailSubject = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->OrderConfirmationMailSubject);
    $OCMailOrderSummaryURL = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->OCMailOrderSummaryURL);
    $BorderRadiusOrder = ($selectSQLecommerceOptions->BorderRadiusOrder==1) ? 'checked' : '';
    $FeControlsStyleWhiteOrder = ($selectSQLecommerceOptions->FeControlsStyleOrder=='white' OR empty($selectSQLecommerceOptions->FeControlsStyleOrder)) ? 'checked' : '';
    $FeControlsStyleBlackOrder = ($selectSQLecommerceOptions->FeControlsStyleOrder=='black') ? 'checked' : '';
    $ForwardAfterPurchaseUrl = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceOptions->ForwardAfterPurchaseUrl);
    $Tax = $selectSQLecommerceOptions->TaxPercentageDefault;
    $Shipping = $selectSQLecommerceOptions->ShippingGross;
    $AllowedCountries = (!empty($selectSQLecommerceOptions->AllowedCountries)) ? unserialize($selectSQLecommerceOptions->AllowedCountries) : [];
    $AllowedCountriesTranslations = (!empty($selectSQLecommerceOptions->AllowedCountriesTranslations)) ? unserialize($selectSQLecommerceOptions->AllowedCountriesTranslations) : [];

    //var_dump('1231256657567');
    //var_dump($Tax);
    if($Tax==0.0){
        $Tax='';
    }
    ...
    $CreateInvoice = ($selectSQLecommerceOptions->CreateInvoice==1) ? 'checked' : '';
    $cg_invoice_disabled = ($CreateInvoice=='checked') ? '' : 'cg_disabled';
    $cg_invoice_disabled_one = ($CreateInvoice=='checked') ? '' : 'cg_disabled_one';
    $SendInvoice = ($selectSQLecommerceOptions->SendInvoice==1) ? 'checked' : '';
    $EnvironmentSandbox = ($selectSQLecommerceOptions->Environment=='sandbox') ? 'checked' : '';
    $EnvironmentLive = ($selectSQLecommerceOptions->Environment=='live') ? 'checked' : '';

    $selectSQLecommerceInvoiceOptions = cg_get_ecommerce_invoice_options();
    $CreateAndSetInvoiceNumber = ($selectSQLecommerceInvoiceOptions->CreateAndSetInvoiceNumber==1) ? 'checked' : '';

    $ResetCustomNumberNextInvoiceTest = ($selectSQLecommerceInvoiceOptions->ResetCustomNumberNextInvoiceTest==1) ? 'checked' : '';

    $ResetCustomNumberNextInvoiceLive = ($selectSQLecommerceInvoiceOptions->ResetCustomNumberNextInvoiceLive==1) ? 'checked' : '';
    $UseNewInvoiceNumberLogic = ($selectSQLecommerceInvoiceOptions->UseNewInvoiceNumberLogic==1) ? 'checked' : '';

    $InvoiceNumberLogicSelectLive = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceInvoiceOptions->InvoiceNumberLogicSelectLive);
    $InvoiceNumberLogicOwnPrefixLive = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceInvoiceOptions->InvoiceNumberLogicOwnPrefixLive);
    $InvoiceNumberLogicCustomNumberLive = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceInvoiceOptions->InvoiceNumberLogicCustomNumberLive);

    $InvoiceNumberLogicSelectTest = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceInvoiceOptions->InvoiceNumberLogicSelectTest);
    $InvoiceNumberLogicOwnPrefixTest = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceInvoiceOptions->InvoiceNumberLogicOwnPrefixTest);
    $InvoiceNumberLogicCustomNumberTest = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLecommerceInvoiceOptions->InvoiceNumberLogicCustomNumberTest);

    $InvoiceNote = unserialize($selectSQLecommerceInvoiceOptions->InvoiceNote);
    if(empty($InvoiceNote)){$InvoiceNote = [];}
    $InvoicerHeaderData = cg_stripslashes_recursively($selectSQLecommerceInvoiceOptions->InvoicerHeaderData);

	// mysql> SELECT * FROM SelectNonEmptyValues WHERE Name IS NOT NULL AND TRIM(Name) <> '';
	$lastInvoiceNumberTest = $wpdb->get_var($wpdb->prepare( "SELECT InvoiceNumber FROM $tablename_ecommerce_orders WHERE id > %d AND IsTest = 1 AND InvoiceNumber IS NOT NULL AND TRIM(InvoiceNumber) <> '' ORDER BY id DESC LIMIT 1",[0]));
	...
	$cg_disabled_override_custom_number_reset_test = '';
	if(empty($lastInvoiceNumberTest)){
		$cg_disabled_override_custom_number_reset_test = 'cg_disabled_override';
		$lastInvoiceNumberTest = 'no invoice with invoice number so far generated';
	}
	$cg_disabled_override_custom_number_test = 'cg_disabled_override';
	$cg_disabled_override_custom_number_input_test = 'disabled';
	$cg_custom_number_generated_note_test = '';
	$cg_originally_configurated_custom_number_test = '';
	if(!empty($InvoiceNumberLogicCustomNumberTest)){
		$cg_originally_configurated_custom_number_test = '<br>Originally configurated custom number: <b>'.$InvoiceNumberLogicCustomNumberTest.'</b>';
	}
	$InvoiceNumberLogicCustomNumberPreviousIncreasedTest = '';
	$InvoiceNumberLogicCustomNumberIsGeneratedTest = 0;
	if(empty($lastInvoiceNumberLogicCustomNumberTest)){
		$cg_disabled_override_custom_number_reset_test = 'cg_disabled_override';
		$cg_disabled_override_custom_number_test = '';
		$cg_disabled_override_custom_number_input_test = '';
		$lastInvoiceNumberLogicCustomNumberTest = 'no invoice with custom number so far generated';
	}else{
		$InvoiceNumberLogicCustomNumberIsGeneratedTest = 1;
		$cg_custom_number_generated_note_test = '<br><span class="cg_font_weight_500 cg_color_red">NOTE:</span> Invoice with a custom number already generated. Check "Reset current custom number logic..." below to to edit.';
		$cg_disabled_override_custom_number_reset_test = '';
		$InvoiceNumberLogicCustomNumberPreviousIncreasedTest = cg_increase_number($lastInvoiceNumberLogicCustomNumberTest);
		$InvoiceNumberLogicCustomNumberPreviousIncreasedTest = $InvoiceNumberLogicCustomNumberPreviousIncreasedTest['InvoiceNumberLogicCustomNumber'];
		if($ResetCustomNumberNextInvoiceTest){
			$cg_disabled_override_custom_number_test = '';
			$cg_disabled_override_custom_number_input_test = '';
		}else{
			$InvoiceNumberLogicCustomNumberTest = cg_increase_number($lastInvoiceNumberLogicCustomNumberTest);
			$InvoiceNumberLogicCustomNumberTest = $InvoiceNumberLogicCustomNumberTest['InvoiceNumberLogicCustomNumber'];
		}
	}

	$lastInvoiceNumberLive = $wpdb->get_var($wpdb->prepare( "SELECT InvoiceNumber FROM $tablename_ecommerce_orders WHERE id > %d AND IsTest = 0 AND InvoiceNumber IS NOT NULL AND TRIM(InvoiceNumber) <> ''  ORDER BY id DESC LIMIT 1",[0]));
	$lastInvoiceNumberLogicCustomNumberLive = $wpdb->get_var($wpdb->prepare( "SELECT InvoiceNumberLogicCustomNumber FROM $tablename_ecommerce_orders WHERE id > %d AND IsTest = 0 AND InvoiceNumberLogicCustomNumber IS NOT NULL AND TRIM(InvoiceNumberLogicCustomNumber) <> '' ORDER BY id DESC LIMIT 1",[0]));

	$InvoiceNumberLogicCustomNumberIsGeneratedLive = 0;
	$cg_disabled_override_custom_number_reset_live = '';
	if(empty($lastInvoiceNumberLive)){
		$cg_disabled_override_custom_number_reset_live = 'cg_disabled_override';
		$lastInvoiceNumberLive = 'no invoice with invoice number generated';
	}
	$cg_disabled_override_custom_number_live = 'cg_disabled_override';
	$cg_disabled_override_custom_number_input_live = 'disabled';
	$cg_custom_number_generated_note_live = '';
	$cg_originally_configurated_custom_number_live = '';
	if(!empty($InvoiceNumberLogicCustomNumberLive)){
		$cg_originally_configurated_custom_number_live = '<br>Originally configurated custom number: <b>'.$InvoiceNumberLogicCustomNumberLive.'</b>';
	}
	$InvoiceNumberLogicCustomNumberPreviousIncreasedLive = '';
	$InvoiceNumberLogicCustomNumberIsGeneratedLive = 0;
	if(empty($lastInvoiceNumberLogicCustomNumberLive)){
		$cg_disabled_override_custom_number_reset_live = 'cg_disabled_override';
		$cg_disabled_override_custom_number_live = '';
		$cg_disabled_override_custom_number_input_live = '';
		$lastInvoiceNumberLogicCustomNumberLive = 'no invoice with custom number so far generated';
	}else{
		$InvoiceNumberLogicCustomNumberIsGeneratedLive = 1;
		$cg_custom_number_generated_note_live = '<br><span class="cg_font_weight_500 cg_color_red">NOTE:</span> Invoice with a custom number already generated. Check "Reset current custom number logic..." below to to edit.';
		$cg_disabled_override_custom_number_reset_live = '';
		$InvoiceNumberLogicCustomNumberPreviousIncreasedLive = cg_increase_number($lastInvoiceNumberLogicCustomNumberLive);
		$InvoiceNumberLogicCustomNumberPreviousIncreasedLive = $InvoiceNumberLogicCustomNumberPreviousIncreasedLive['InvoiceNumberLogicCustomNumber'];
		if($ResetCustomNumberNextInvoiceLive){
			$cg_disabled_override_custom_number_live = '';
			$cg_disabled_override_custom_number_input_live = '';
		}else{
			$InvoiceNumberLogicCustomNumberLive = cg_increase_number($lastInvoiceNumberLogicCustomNumberLive);
			$InvoiceNumberLogicCustomNumberLive = $InvoiceNumberLogicCustomNumberLive['InvoiceNumberLogicCustomNumber'];
		}
	}
...
foreach($selectSQL4 as $value4){
	...
	$ConsentYoutube = ($value4->ConsentYoutube==1) ? 'checked' : '';
...
$WpPageParentWinner = $selectSQL1 ->WpPageParentWinner;
    ...
    $WpPageParentEcommercePermalink = ($WpPageParentEcommerce) ? get_permalink($WpPageParentEcommerce) : '';
...
if(empty($jsonOptions[$galeryID.'-u'])){
    ...
    $jsonOptionsNew[$galeryNR.'-ec'] = $jsonOptions;

    $jsonOptions = $jsonOptionsNew;
    $isModernOptionsNew = true;

}else{// has to be done extra, was added later 2022-06-12
    if(empty($jsonOptions[$galeryNR.'-ec'])){
        $jsonOptions[$galeryNR.'-ec'] = $jsonOptions[$galeryNR.'-w'];
    }
}
...
// get mail exception logs general
$mailExceptionsGeneral = '';
...
if(!empty($_POST['cg_edit_translations']) || !empty($_GET['cg_edit_translations'])){
    echo '<input type="hidden" name="cg_edit_translations" class="cg_edit_translations" value="true" >';
}

if(!empty($_POST['cg_edit_ecommerce']) || !empty($_GET['cg_edit_ecommerce'])){
    echo '<input type="hidden" name="cg_edit_ecommerce" class="cg_edit_ecommerce" value="true" >';
}
