...
"Automatically activate users entries after successful frontend contact" can be activated/deactivated in "Edit options" >>> "Contact options"
	...
	$MainTitleGalleriesView = 'Main title gallery '.$nextIDgallery;
	$SubTitleGalleriesView = 'Sub title gallery '.$nextIDgallery;
    ...
    $wpdb->query( $wpdb->prepare(
        "
							INSERT INTO $tablename_form_input
							( id, GalleryID, Field_Type, Field_Order, Field_Content,Show_Slider,Use_as_URL,Active,IsForWpPageTitle,EcommerceTitle)
							VALUES ( %s,%d,%s,%d,%s,%d,%d,%d,%d,%d )
						",
        '',$nextIDgallery,'text-f',2,$nfFieldsArray,1,0,1,1,1
    ) );
