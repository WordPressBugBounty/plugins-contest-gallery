...
if(!empty($GalleryID)){// in case very old gallery created before 14
...
if(empty($galleryDbVersion) || intval($galleryDbVersion)>=14){
    $registry_and_login_options = $wpdb->get_row("SELECT RegistryUserRole, TextBeforeRegFormBeforeLoggedIn FROM $tablename_registry_and_login_options WHERE GeneralID='1'");//get row
        $RegistryUserRole = $registry_and_login_options->RegistryUserRole;
        $galleryDbVersion = cg_get_db_version();
}else{// then $GalleryID must be sent
