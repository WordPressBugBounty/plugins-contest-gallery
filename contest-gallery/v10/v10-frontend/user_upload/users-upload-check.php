...
if(empty($_POST["cg_upload_action"]) OR empty($_FILES["data"])){
    ...
    if(!empty($RegUserUploadOnly) && empty($OrderItem)){
        ...
        }elseif($RegUserUploadOnly==3 && !empty($RegUserMaxUpload)){
            $isCountCheckHasToBeDone = true;
        ...
        foreach ($files['name'] as $key => $value) {
            ...
            //if ($isOnlyContactEntry || $files['size'] > 0) {
                ...
                if(!$isOnlyContactEntry){
                    ...
                    elseif($post_type=="image/jpg"){$post_type="jpg";$imageType="jpg";}
                    elseif($post_type=="image/x-citrix-jpeg"){$post_type="jpg";$imageType="jpg";}// appears only  if file post with attach id already generated by wordpress, other cases so far have not to be proved
                    elseif($post_type=="image/x-citrix-png"){$post_type="png";$imageType="png";}// appears only  if file post with attach id already generated by wordpress, other cases so far have not to be proved
                    elseif($post_type=="image/x-citrix-gif"){$post_type="gif";$imageType="gif";}// appears only  if file post with attach id already generated by wordpress, other cases so far have not to be proved
                    elseif($post_type=="image/png"){$post_type="png";$imageType="png";}
                    elseif($post_type=="image/gif"){$post_type="gif";$imageType="gif";}
                    elseif($post_type=="application/pdf"){$post_type="pdf";$imageType="pdf";}
                    elseif($post_type=="application/zip"){$post_type="zip";$imageType="zip";}
                    elseif($post_type=="text/plain"){$post_type="txt";$imageType="txt";}
                    elseif($post_type=="application/msword"){$post_type="doc";$imageType="doc";}
                    elseif($post_type=="application/vnd.openxmlformats-officedocument.wordprocessingml.document"){$post_type="docx";$imageType="docx";}
                    elseif($post_type=="application/vnd.ms-excel"){$post_type="xls";$imageType="xls";}
                    elseif($post_type=="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"){$post_type="xlsx";$imageType="xlsx";}
                    elseif($post_type=="text/csv"){$post_type="csv";$imageType="csv";}
                    elseif($post_type=="audio/mpeg" && $type=="audio/mpeg"){$post_type="mp3";$imageType="mp3";}
                    elseif($post_type=="audio/mpeg" && $type=="audio/x-m4a"){$post_type="m4a";$imageType="m4a";}
                    elseif($post_type=="audio/ogg"){$post_type="ogg";$imageType="ogg";}
                    elseif($post_type=="audio/wav"){$post_type="wav";$imageType="wav";}
                    elseif($post_type=="video/mp4"){$post_type="mp4";$imageType="mp4";}
                    //elseif($post_type=="video/avi"){$post_type="avi";$imageType="avi";}
                    elseif($post_type=="video/quicktime"){$post_type="mov";$imageType="mov";}
                    elseif($post_type=="video/webm"){$post_type="webm";$imageType="webm";}
                    elseif($post_type=="application/vnd.ms-powerpoint"){$post_type="ppt";$imageType="ppt";}
                    elseif($post_type=="application/vnd.openxmlformats-officedocument.presentationml.presentation"){$post_type="pptx";$imageType="pptx";}
                    //elseif($post_type=="video/x-ms-wmv"){$post_type="wmv";$imageType="wmv";}
                    else{
            ...
            //    var_dump('$fileType');
                ...
                if($RegUserUploadOnly==1){
                    $CheckSet = 'CheckLogin';
                }elseif($RegUserUploadOnly==2){
                    $CheckSet = 'CheckCookie';
                }elseif($RegUserUploadOnly==3){
                    $CheckSet = 'CheckIp';
                }
...
/*
                        ...
                        foreach ($form_input as $key => $value) {
                            ...
                            if ($i==4 AND ($ft=='nf' or $ft=='ef' or $ft=='se' or $ft=='ra' or $ft=='chk' or $ft=='url' or $ft=='sec' or $ft=='cb' or $ft=='fbt' or $ft=='dt')){
                                ...
                                if($ft=='ra'){
                                    ...
                                    VALUES ( %s,%d,%d,%d,%s,%d,%s,%s,%d )
                                ",
                                        '',$nextId,$f_input_id,$galeryID,'radio-f',$field_order,$content,'',$inputsTime
                                    ) );
                                    $inputContents[$wpdb->insert_id] = $content;
                                ...
                                if($ft=='chk'){
                                    ...
                                    VALUES ( %s,%d,%d,%d,%s,%d,%s,%s,%d )
                                ",
                                        '',$nextId,$f_input_id,$galeryID,'chk-f',$field_order,$content,'',$inputsTime
                                    ) );
                                    $inputContents[$wpdb->insert_id] = $content;
                ...
                if(!$post_title){
                    $post_title = 'entry';// if ImgType con
                }
                ...
                if(!empty($selectSQL1->WpPageParent)){
                    ...
                    if(strpos($galeryIDuser,'-uf')!==false || strpos($galeryIDuser,'-cf')!==false){
                        ...
                        }elseif(strpos($galeryIDuser,'-ec')!==false && intval($cgVersion)>=22){
        ...
        if($ActivateUpload==1){
            ...
            if(!empty($picsSQL)){
                ...
                foreach ($imagesArray as $pid => $imagesArrayValues){
	                $pidsArray[] = $pid;
                    if(!empty($WpUserId)){
                        $imagesArray[$pid]['WpUserId'] = $WpUserId;// will not be set in JSON files
                    }
                }
