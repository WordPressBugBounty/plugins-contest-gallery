...
echo "<div id='cgMinimizeUploadForm$galeryIDuserForJs' class='cg-minimize-upload-form cg_hide cg_hide_override $cgFeControlsStyle' data-cg-gid='$galeryIDuserForJs' data-cg-tooltip='$language_MinimizeUploadForm'>";
echo "</div>";
...
echo "<div class='cg_upload_submit_skeleton_panel'>";
echo "<div class='cg_upload_submit_skeleton_header'>";
echo "<div class='cg_skeleton_loader_on_page_load cg_upload_submit_skeleton_title'></div>";
echo "<div class='cg_skeleton_loader_on_page_load cg_upload_submit_skeleton_badge'></div>";
echo "</div>";
echo "<div class='cg_upload_submit_skeleton_file'>";
echo "<div class='cg_skeleton_loader_on_page_load cg_upload_submit_skeleton_icon'></div>";
echo "<div class='cg_upload_submit_skeleton_file_lines'>";
echo "<div class='cg_skeleton_loader_on_page_load cg_upload_submit_skeleton_line cg_upload_submit_skeleton_line_wide'></div>";
echo "<div class='cg_skeleton_loader_on_page_load cg_upload_submit_skeleton_line'></div>";
echo "</div>";
echo "</div>";
echo "<div class='cg_upload_submit_skeleton_progress'>";
echo "<div class='cg_skeleton_loader_on_page_load cg_upload_submit_skeleton_progress_bar'></div>";
echo "<div class='cg_skeleton_loader_on_page_load cg_upload_submit_skeleton_progress_meta'></div>";
echo "</div>";
echo "</div>";
echo "</div>";
...
if ((time() >= $ContestEndTime && $ContestEnd == 1) or $ContestEnd == 2) {
    ...
    echo "</div>";
} elseif (time() < $ContestStartTime && $ContestStart == 1) {
    ...
    if ($options['pro']['RegUserUploadOnly'] == 1 && is_user_logged_in() == false) {
        $regUserOnlyHide = 'cg_hide';
        echo '<div class="mainCGdivUploadRegUserOnlyText">';
            echo contest_gal1ery_convert_for_html_output_without_nl2br($options['pro']['RegUserUploadOnlyText']);
        echo '</div>';
    }

    echo '<div class="mainCGdivUploadContent '.$regUserOnlyHide.'" >';
        ...
        if (empty($isOnlyContactForm)) {// for in gallery upload form
            ...
            echo contest_gal1ery_convert_for_html_output_without_nl2br($options['pro']['GalleryUploadTextBefore']);
            echo "</div>";
        }
            ...
            if (!empty($jsonUploadForm)) {
                foreach ($jsonUploadForm as $fieldOrder => $field) {
                    ...
                    if ($field['RowNumber'] == 0) {// if first time load in 27.0.0
                        ...
                        $echoRowClosed = true;
                    } elseif ($field['RowNumber'] != $previousRowNumber) {
                        echo "<div class='cg_row'>";
                        $echoRowClosed = false;
                    }
                    ...
                    if ($field['Field_Type'] == 'radio-f' && $field['Active'] == '1') {
                        $fieldId = $field['id'];
                        $Field_Order = $field['Field_Order'];
                        $Field_Content = $field['Field_Content'];
                        $checkIfNeed = $Field_Content['mandatory'];

                        $necessary = '*';//radio is always required
                        $checkIfNeed = ($Field_Content['mandatory'] == 'on') ? 'on' : '';
                        ...
                        echo "<div class='cg_radio'>";

                        $rI = 1;
                        foreach ($textAr as $key => $value) {
                            $value = sanitize_text_field(contest_gal1ery_convert_for_html_output_without_nl2br($value));
                            echo '<div class="cg_radio_button">
                                    <label for="cg_radio_button_'.$fieldId.$rI.'">'.$value.'</label>
                                    <input type="radio" id="cg_radio_button_'.$fieldId.$rI.'" name="form_input['.$Field_Order.'][]" value="'.$value.'" class="cg_radio_button_input">
                                </div>';
                            $rI++;
                        }
                    ...
                    if ($field['Field_Type'] == 'chk-f' && $field['Active'] == '1') {
                        $fieldId = $field['id'];
                        $Field_Order = $field['Field_Order'];
                        $Field_Content = $field['Field_Content'];
                        $checkIfNeed = $Field_Content['mandatory'];
                        ...
                        echo "<div class='cg_check'>";

                        $rI = 1;
                        foreach ($textAr as $key => $value) {
                            $value = sanitize_text_field(contest_gal1ery_convert_for_html_output_without_nl2br($value));
                            echo '<div class="cg_check_button">
                                    <label for="cg_check_button_'.$fieldId.$rI.'">'.$value.'</label>
                                    <input type="checkbox" id="cg_check_button_'.$fieldId.$rI.'" class="cg_check_button_input" value="'.$value.'">
                                </div>';
                            $rI++;
                        }
        ...
        if (empty($isOnlyContactForm)) {// then in gallery upload form
            ...
            echo contest_gal1ery_convert_for_html_output_without_nl2br($options['pro']['GalleryUploadTextAfter']);
            echo "</div>";
        }
...
echo "</div>";
if (!empty($isOnlyUploadFormPayPal)) {

    $selectSQLpaypalUploadOptions = $wpdb->get_row("SELECT * FROM $tablename_paypal_upload_options WHERE GalleryID = $realGid");
    $Price = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLpaypalUploadOptions->Price);
    $SaleTitlePayPalUpload = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLpaypalUploadOptions->SaleTitle);
    $SaleDescriptionPayPalUpload = contest_gal1ery_convert_for_html_output_without_nl2br($selectSQLpaypalUploadOptions->SaleDescription);

    echo "<div class='mainCGdivUploadFormPayPalButton $cgFeControlsStyle $BorderRadiusClassUploadForm' id='mainCGdivUploadFormPayPalButton$galeryIDuser' data-cg-gid='$galeryIDuser'>";
    echo "PayPal button here!!!";
    echo "<input type='hidden' class='mainCGdivUploadFormPayPalPrice' id='mainCGdivUploadFormPayPalSaleTitle$galeryIDuser' value='$Price' >";
    echo "<input type='hidden' class='mainCGdivUploadFormPayPalSaleTitle' id='mainCGdivUploadFormPayPalSaleTitle$galeryIDuser' value='$SaleTitlePayPalUpload' >";
    echo "<input type='hidden' class='mainCGdivUploadFormPayPalSaleDescription' id='mainCGdivUploadFormPayPalSaleDescription$galeryIDuser' value='$SaleDescriptionPayPalUpload' >";
    echo "</div>";
}
echo "</div>";
